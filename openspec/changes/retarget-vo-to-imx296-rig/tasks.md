...
- [x] 3.3b **IMU source built: `bev_imu/imu_node`.** Wraps the J106 reader (mounted at `/opt/j106-tools`) rather than reimplementing the edge handling, and publishes `sensor_msgs/Imu` on `/imu0` stamped at the data-ready edge on `CLOCK_MONOTONIC` — the same clock as the cameras, so images and IMU land in one bag on one timebase. Verified live: WHO_AM_I 0x71, 200.28 Hz, gyro ±500 dps / accel ±4 g, DLPF group delays logged (gyro lags accel 1.02 ms) and deliberately **not** applied, Δ logged UNMEASURED.

  **A Python GC pause was eating IMU samples**: a stalled loop misses data-ready edges outright — one 78 ms gap and 29 lost samples in 18 s. With `gc.freeze()`/`gc.disable()` after startup plus a reused message object: sd 1251 → 46 µs, max interval 78414 → 5073 µs, **0 missed edges in 4061 samples**. Reference (the bare reader, no ROS) is sd 15.7 µs, so the ROS layer costs ~30 µs of extra stamp jitter — inside the 0.1–1 ms Δ is resolved to, but use the bare reader for a reference-grade calibration run.

  Needs `privileged` (device cgroup for `/dev/spidev1.0` + `/dev/gpiochip*`, `CAP_SYS_NICE` for `SCHED_FIFO`); the compose `imu` service sets it.
