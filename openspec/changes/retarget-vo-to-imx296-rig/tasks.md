...
- [~] 3.1 **Script ready: `scripts/calib/record_calib_session.sh`.** Prompts the eight stages in ring order, runs capture (decimated) + IMU + `ros2 bag record` in one container, and writes provenance. Awaiting the session itself. Record on the board: AprilGrid through all four fields of view **including the adjacent overlaps**, then an IMU excitation sequence (rotation about all three axes, then translation). `ros2 bag` at ~4 Hz images + full-rate `/imu0`; stop `systemd-timesyncd` first (it cuts the frame-time fit residual 30.9 µs → 8.4 µs).
...
- [ ] 3.2c **Persist `trigger_mode=1` across reboots.** Found the hard way mid-session: the board rebooted, `trigger_mode` reset to its default 0, the STM32 kept pulsing, the cameras ignored it and free-ran — **no error anywhere**, only AE gain-hunting (luma p2p 60–97 vs 0.5) and sets that are not sets. `imx296` is built into the Image, not a module, so `modprobe.d` will not do it: it needs `imx296.trigger_mode=1` on the kernel command line in the extlinux entry, or a small boot service. Board-config change, so it belongs to the J106 project — until then the record script's preflight catches it.
...
- [ ] 3.5 **Stage 2 — extrinsics for adjacent PAIRS in RING order** (`cam1 → cam2 → cam4 → cam3`, i.e. ports c→d→f→e; cam2/cam3 are diagonal and share no baseline), sequentially, then compose around the ring and check the loop-closure residual (spec: *Extrinsics are consistent around the rig*). A pair that will not solve is evidence about overlap, not just a failed run — it is the same question §5.3 asks of cuVSLAM's frustum gate.
