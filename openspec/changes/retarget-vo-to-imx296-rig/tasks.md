...
- [x] 3.7 **Stage 4 done — four virtual stereo pairs generated and measured.** Each fisheye carved into two virtual pinholes at ±45°, 100° each; the facing pair is *derived* from the measured extrinsic (optical axes 1.1–2.1° apart) rather than assumed. Config in `config/rig/virtual_stereo_imx296.yaml`, generator + checker in `scripts/calib/`.

  | pair | corners | median \|dy\| | rms | disparity | depth |
  |---|---|---|---|---|---|
  | left | 3996 | **0.87 px** | 3.05 | 171.6 px, 100 % consistent | 0.46 m |
  | front | 2944 | **1.63 px** | 3.30 | 195.2 px, 100 % | 0.41 m |
  | right | 3788 | **0.71 px** | 3.88 | 178.3 px, 100 % | 0.45 m |
  | rear | 3888 | **0.93 px** | 2.22 | 190.8 px, 100 % | 0.42 m |

  Usable, not excellent — a good physical stereo rig reaches under 0.5 px, so disparity search needs a few pixels of vertical tolerance. The residual is *consistent with its inputs*: at focal 537 px one degree of rotation error is 9.4 px, so ~1 px ≈ 0.1°, inside each pair's 0.55–0.68° angular spread. Improving it means a better extrinsic, not a different rectification.

  Two measurement lessons: **resolution is not free** — at 480×360 the virtual focal is 0.38× the fisheye's near-axis scale and tag detection collapsed from ~10/frame to 0.1; 1280×960 gives 1.00×. And **ORB-based epipolar measurement is worthless here** — it reported 4.15 px median / 74 px p90 on a repetitive indoor scene, measuring the matcher rather than the rig; tag identity removes the ambiguity.
