# fused-vo Specification

## Purpose

Single-process, zero-copy visual odometry: capture the 4 Argus cameras and run cuVSLAM `Track()` in
one process, feeding each frame's NVMM luma plane to cuVSLAM as GPU memory (EGL→CUDA bridge) — no
host pixel copy, no DDS image transport on the tracking path.
